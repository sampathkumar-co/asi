...
Post-v31 vNext work first hardens the campaign boundary: provider/transport and trusted-runner failures are explicit execution incidents, `clean_execution` is mandatory for promotion, and trusted mechanical-support defects cannot enter the model-repair path. Current development validation is **163/163 tests** and **48/48 Gate-2 canaries**. Only after this and any further generic public/dev diagnostics are frozen and CI-green may a completely fresh private/OOD holdout be created. The next generic step adds bounded task-independent JSON Schemas to normal Raw/Seed/verifier Ollama calls. That tree passes **168/168 tests** and **54/54 canaries** and completes the eight-task public suite with zero repairs/incidents, Raw **0.821875**, Seed **0.98750**, gain **+0.165625**, CI **[+0.06250, +0.303125]**.
