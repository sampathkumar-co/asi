...
Gate 2 remains uncertified after private v1, v2, and v3.1. Do not rerun or tune on any retired holdout. Post-v31 execution-integrity, structured-output, and provider-preflight hardening are development-only and are evaluated only on public/dev data. Once the new candidate is finalized, freeze it, require green CI, create a completely new external >=16-pair private/OOD holdout after the freeze, preregister it, require preregistration CI, and only then infer.
