...
Post-v31 vNext development adds explicit execution-integrity failure classes. Provider/transport and trusted-runner defects invalidate the campaign through a mandatory `clean_execution` check; model/protocol failures and budget exhaustion continue to fail closed at arm level. The current development tree passes **163/163 tests** and **48/48 deterministic Gate-2 canaries**. This hardening is not itself empirical certification. Purpose-specific, task-independent JSON Schemas now constrain normal Gate-2 Ollama outputs without encoding scientific IDs or answers; stage-dependent repair remains generic. Current validation is **168/168 tests** and **54/54 deterministic canaries**. Public development remains non-certifying: Raw **0.821875**, Seed **0.98750**, gain **+0.165625**, CI **[+0.06250, +0.303125]**, zero repairs/incidents.
