...
The final generic infrastructure boundary before a new candidate freeze adds a fixed provider-readiness probe before the first unfinished scored pair. The prompt contains no task, holdout, hypothesis, experiment, outcome, answer-key, or private-evidence content. The policy is versioned and attested in campaign settings, uses purpose `gate2_preflight` with an **8-token** output cap, runs outside the common Raw/Seed scored envelope, and has no automatic retry.
