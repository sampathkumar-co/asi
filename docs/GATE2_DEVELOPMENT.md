...
## Post-v31 vNext provider-readiness preflight

The next generic infrastructure boundary adds a fixed provider-readiness probe before the first unfinished scored pair. The prompt contains no task, holdout, hypothesis, experiment, outcome, answer-key, or private-evidence content. The policy is versioned and attested in campaign settings, uses purpose `gate2_preflight` with an **8-token** output cap, runs outside the common Raw/Seed scored envelope, and has no automatic retry.

Checkpoint and task identity are validated before the probe. A completed resume skips it; an unfinished resume probes again before the next pair. On success the journal records only the prompt hash, response hash, and token counts. On provider failure it records the prompt hash plus a hash of the failure detail, raises immediately, and creates no scored pair.

At the provider-preflight milestone, development validation was **176/176 tests PASS**, compileall and diff hygiene PASS, and **56/56 Gate-2 deterministic canaries PASS**. Qualification hash: `bf52163881fcf6519812cac89e6f0c53361471efe60d1987c79dd679ced57369`. Development implementation digest: `e8b8d4f15aa32babe13dc8640bf9a0df81d82a3ee1066f476757a52aae408785`. A live `qwen3:8b` public-task smoke sealed with `provider_preflight_passed` before `pair_started`; Raw and Seed both succeeded, Seed verifier acceptance was true, and there were zero repairs or failure classifications. This remains development evidence only and does not authorize reuse of a retired holdout.

## Post-v31 vNext bounded schema-repair hardening

The next generic reliability change keeps `gate2_repair` stage-generic but permits a second schema-repair call when the first repaired object is still invalid. The allowance is **two schema-repair calls total per arm**, shared across Raw/Seed attribution, action, design, final-report, and verifier stages. A third schema repair is never attempted. Both repair attempts remain ordinary model calls inside the existing common envelope, and verifier repairs now use the same shared counter and are recorded in `repair_events`.
