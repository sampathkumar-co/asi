...
def weighted_dag_longest_path(
    ...
    checks = {
        ...
        "dynamic_program_valid": all(
            score[node]
            == clean_weights[node] + (max(score[p] for p in preds[node]) if preds[node] else 0)
            for node in topo
        ),
    }
    return {
...
def dag_longest_path_tool(payload: dict[str, Any]) -> ToolResult:
    try:
        ...
        answer = template.replace("{weight}", str(graph["weight"])).replace(
            "{path}", separator.join(graph["path"])
        )
        return ToolResult(
            True,
            output={
