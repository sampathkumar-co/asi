...
class LineageStore:
    ...
    def __init__(self, path: str | Path = "seed-lineage.db") -> None:
        ...
        with self._connect() as con:
            con.execute("""CREATE TABLE IF NOT EXISTS candidates (
                candidate_id TEXT PRIMARY KEY, parent_id TEXT, proposal_id TEXT NOT NULL,
                code_hash TEXT NOT NULL, created_at TEXT NOT NULL)""")
            con.execute("""CREATE TABLE IF NOT EXISTS lineage_events (
    ...
    def _connect(self) -> sqlite3.Connection:
        if self._con is None:
            raise RuntimeError("LineageStore is closed")
        return self._con
    ...
    def add_candidate(self, candidate_id: str, parent_id: str | None, proposal_id: str, code_hash: str) -> CandidateNode:
        ...
        with self._connect() as con:
            con.execute("INSERT INTO candidates(candidate_id,parent_id,proposal_id,code_hash,created_at) VALUES(?,?,?,?,?)",
    ...
    def record(self, candidate_id: str, kind: str, payload: dict[str, Any]) -> None:
        ...
        with self._connect() as con:
            con.execute("INSERT INTO lineage_events(candidate_id,kind,payload,created_at) VALUES(?,?,?,?)",
