...
class BaselineAgent:
    """Gate-1 observe/act/critic loop with explicit budgets and fail-closed errors."""
    ...
    def _stop(self, state: AgentState, status: RunStatus, reason: str) -> AgentState:
        state.status = status
        state.notes.append(reason)
        self._event(state, "run_stopped", {"status": status.value, "reason": reason, "usage": self.budget.usage()})
        return state

    def run(self, goal: Goal) -> AgentState:
        state = AgentState(goal=goal)
        self._event(state, "run_started", {"goal": goal.description, "budget": self.budget.limits()})
        ...
        while self.budget.can_step():
            try:
                ...
                if critique.done:
                    ...
                    self._event(state, "run_completed", {"answer": state.final_answer, "usage": self.budget.usage()})
                    return state
            except BudgetExceeded as exc:
                return self._stop(state, RunStatus.BUDGET_EXHAUSTED, str(exc))
            except PermissionError as exc:
                return self._stop(state, RunStatus.BLOCKED, str(exc))
            except Exception as exc:
                return self._stop(state, RunStatus.FAILED, f"{type(exc).__name__}: {exc}")

        return self._stop(state, RunStatus.BUDGET_EXHAUSTED, "step budget exhausted")
